...
const getRegExp = (html, pubDate) => {
  ...
  if (html.includes('Bislang')) {
    return /Bislang \(Stand (?<date>[a-zäA-Z0-9., ]+)\) wurden in München insgesamt(?: rund)? (?<totalDosesAdministered>[\d.]+) Impfungen durchgeführt \((?<totalFirstDosesAdministered>[\d.]+) Erst- und (?<totalSecondDosesAdministered>[\d.]+) (?:&nbsp;)?Zweitimpfungen\)\. (?:<strong>Auf Beschluss des Bund-Länder-Impfgipfels können sich ab heute Kinder und Jugendliche bereits ab 12 Jahren gegen Corona impfen lassen, so dass die impffähige Münchner Bevölkerung von bislang 1\.270\.488 auf jetzt 1\.316\.385 Personen angewachsen ist\. <\/strong>|Nach der aktuellen Empfehlung der Ständigen Impfkommission \(STIKO\) soll die Corona-Schutzimpfung allen Personen ab 18 Jahren angeboten werden\. )?Die Münchner <strong>Impfquote<\/strong> liegt damit, bezogen auf die (?:impffähige )?Bevölkerung ab 1[268] Jahren, bei den Erstimpfungen bei (?<percentageVaccinableFirstDose>[0-9,]+) % und bei den Zweitimpfungen bei (?<percentageVaccinableSecondDose>[0-9,]+) % \(Münchner Gesamtbevölkerung (?<percentageFirstDose>[0-9,]+) % \/ (?<percentageSecondDose>[0-9,]+) %\)\./;
  }
